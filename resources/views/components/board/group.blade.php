...
<div class="bg-base-200 p-2 rounded-md" wire:key="group-{{ $status }}">
    ...
    >
        @if($items->count() == 0)
            <div wire:key="opportunity-null" class="h-10 border-dashed border-gray-400 border-2 shadow-sm rounded-sm w-full flex items-center justify-center opacity-20">
                Empty list
            </div>
        @endif
